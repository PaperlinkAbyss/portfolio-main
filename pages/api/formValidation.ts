...
export default function (req: NextApiRequest) {
  ...
  if (typeof req.body === 'string') {
    ...
    const jsonNames2 = jsonNames.filter((element) => {
      const [field, value] = element as [Field, string]
  ...
  const response = (Object.entries(req.body) as [Field, string][]).filter(([key, value]) => {
