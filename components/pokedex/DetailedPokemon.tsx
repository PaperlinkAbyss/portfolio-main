...
import bulbasaur, { Abilities, Bulbasaur, ID, Stats, Types, type Sprites } from './initialPokemon'
...
export default function DetailedPokemon({ selectedPokemon }: DetailedPokemon) {
   ...
   */
  const [currentPokemon, setCurrentPokemon] = useState<Bulbasaur | StatePokemon>(bulbasaur)
  ...
  useEffect(() => {
    fetch(`https://pokeapi.co/api/v2/pokemon/${selectedPokemon}`)
      ...
      .then(({ name, height, types, abilities, stats, sprites, id }: StatePokemon) => {
        setCurrentPokemon({
          name,
          height,
          types,
  ...
  return (
    ...
    >
      ...
      <main className='mt-5 flex flex-col text-center '>
        ...
        <p>
          <strong className='font-bold '>{currentPokemon.types[1] ? 'Types: ' : 'Type: '}</strong>
          {currentPokemon.types[1]
            ? `${toCap(currentPokemon.types[0].type.name)},  ${currentPokemon.types[1].type.name}`
            : toCap(currentPokemon.types[0].type.name)}
        </p>
        <p>
          <strong className='font-bold '>
            {currentPokemon.abilities[1] ? 'Abilities: ' : 'Ability: '}
          </strong>
        ...
        <button
          ...
          className='mt-5 w-fit self-center rounded-md border-2 border-cyan-600 bg-cyan-200 bg-opacity-70 p-2 pl-4 pr-4 font-extrabold hover:bg-cyan-500 md:hidden'
        >
          Close
        </button>
      </main>
