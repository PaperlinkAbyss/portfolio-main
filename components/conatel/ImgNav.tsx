...
export default function ImgNav({ img }: { img: ImgType }) {
  const cols = `grid-cols-${img.length} lg:grid-cols-${
    img.length < 4 ? img.length : 3
  } xl:grid-cols-${img.length} md:grid-cols-${
    img.length < 4 ? img.length : 3
  } sm:hidden xl:grid-cols-${img.length < 4 ? img.length : 2}  2xl:grid-cols-${
        ...
        {img.map(({ text, imgSrc, href }) => {
          return (
            <a
              key={href}
              href={href}
              className='flex-cols px-auto m-0 py-4 text-center'
            >
              <Image
